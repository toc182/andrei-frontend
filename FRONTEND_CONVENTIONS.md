...
- Both live in the **gear menu** next to Editar, disabled while the desglose is empty, and act on **what is on screen** — unsaved edits included (what you see is what exports).
- **Exportar a Excel** (`src/lib/desgloseExcel.ts`): builds a real `.xlsx` in the browser. `exceljs` is imported **dynamically** so the app bundle never pays its ~1 MB — keep it that way. Visual parity with the table: group bands (light-theme `--color-grupo-*` as ARGB), bold + indented descriptions, containers with blank montos and dimmed derived total, Subtotal/ITBMS/Total block at the end. Errors surface in the same Alert strip as save errors; success has no message (the download is the feedback).
- **Imprimir / PDF** (`src/lib/desglosePrint.ts` + `DesglosePrintDialog.tsx`): same pipeline as the cronograma print (§20) — pure mm-space SVG page builder + `openPrintWindow`, sharing `fitPrintHeader`/`PRINT_PAPERS`/`PRINT_FONTS` and the logo plumbing (`src/lib/printLogos.ts`). Differences: no pages-wide windows (Descripción absorbs the slack; `errTableTooWide` if even that is too narrow), descriptions **wrap ≤4 lines** so rows have variable height and pagination packs by height. "Máx. páginas de alto" re-derives the ENTIRE layout per 0.5pt font step (wraps and columns move with the font — never scale), floor 6pt, `layout.warn` when it still doesn't fit. **No persistence** in v1 — every open starts from defaults.
- Print **design "B′v2"** (approved by Ivan over a mock, 2026-07-20): the TABLE is framed with a subtle border (`#c9cfd8`, **1.5mm rounded corners** — band fills clip to the radius via clipPath) — never the full page —, sitting **5mm below** the document header. Header band `#9cb0d0` (one step darker than `--color-grupo-0`) with labels **vertically centered**; **no vertical column lines**; light horizontal hairlines between rows; blue depth bands for sections. Subtotal/ITBMS/Total are **rows of the table, all bold** (labels in the P.U. column, amounts in Total), inside the same frame — they get their own framed block only when they overflow to an extra page. Footer: "Impreso: {fecha}" left (fecha comes from the caller — the builder never reads the clock) + "Pág X de Y" right. Alignment: Item/Unidad/Cantidad centered (content + header), P.U./Total amounts right-aligned under centered headers, Descripción left — the Excel export mirrors this alignment exactly.
- The print dialog is **two-pane with a live preview** (`size="complex"`): settings left (papel, orientación — vertical default —, letra, máx. páginas, título defaulting to the project name, subtítulo, logos 3/side; margen tucked under a "Más opciones" toggle), preview right rendering the builder's own SVG pages scaled down (first 4 + "…y N más"), rebuilt debounced 250ms behind a sequence guard. The preview IS the printed output — never draw a separate approximation. Bundled logo assets resolve once per session (`printLogos.ts` caches the promise).
- The print builder is pure and gated by `scripts/desglose-print.spec.ts` (`npx tsx scripts/desglose-print.spec.ts`) — golden totals come from the Chagres QA fixture. Add cases before changing layout math.
- **Print page blocks are sized in `%`, never fixed mm** (`buildPrintDocument` in `cronogramaPrint.ts`, shared by both printers). A fixed `height: <printable>mm` is a knife edge: any printer whose usable area is a hair shorter than the CSS promises emits a BLANK page after every real one (andrei-backend#67 — 2 pages printed as 4). Percentages resolve against the real page box and the SVG viewBox letterboxes to fit. Gated in `scripts/cronograma-print.spec.ts`.

### Desgloses de Cuentas (varios por proyecto)

- A project has ONE `tipo='oficial'` desglose (Información, §21) **plus any number of `tipo='cuentas'` ones** under Cuentas → Desgloses: the detailed one the institution asks for, the post-design one, the progress-support one. Backend: migration 142 (`fecha`, `copiado_de_id`, `desglose_comentarios`) and `routes/desgloses.ts` `/proyecto/:id/cuentas*`. The migration-140 partial unique index still guarantees one active oficial per project — its `WHERE` only covers `tipo='oficial'`.
- **Creating**: blank, or **copying** any active desglose of the same project (the oficial included — `/fuentes` lists them with row counts). The copy is a **one-time snapshot**: rows are duplicated with the parent/child tree remapped inside the copy, and editing it never touches the origin. `copiado_de_id` records provenance and is `ON DELETE SET NULL`. Comments are NOT copied — they belong to the desglose they were written on.
- **`DesgloseView` is reused verbatim** for these: pass `desgloseId` and it loads/saves that document via `/cuentas/:desgloseId`; omit it and it is the official desglose exactly as before. Same editor, ITBMS, paste, print and Excel. `titulo` sets the heading, `onBack` renders the loose return arrow (`Undo2`, 18px, no box, ghost hover) left of the title — `SectionHeader`'s own `mb-4` must be neutralized (`[&>div]:mb-0`) or the arrow sits 8px low.
...
- **Un solo `MoreVertical` por card**: el de la cabecera, que reúne las acciones de la sección entera. Los campos que solo se usan a veces (escribir una actualización) se abren desde ahí en vez de ocupar sitio permanentemente. Dos disparadores con el mismo icono a pocos píxeles uno del otro — uno en la cabecera y otro por fila — se leen como un error de la pantalla, no como dos menús distintos.
...
- **Las acciones de una fila viven dentro de la fila abierta**, no en chrome que aparece al hover: una hilera de `Button` `variant="ghost"` con `h-auto px-0 py-0 text-xs`, icono `h-3 w-3` y el mismo sangrado de 2.5 que el detalle. Van al final del bloque expandido y se muestran aunque el evento no tenga detalle que enseñar. Cada botón lleva `stopPropagation` porque la fila entera abre y cierra al hacer clic. Reference: `src/pages/cuentas/CuentaTimeline.tsx`.
