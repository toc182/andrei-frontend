...
function getStatusConfig(tipo: string, estado: string) {
  if (tipo === 'licitacion') {
    return (
      LICITACION_ESTADOS[estado] || { label: estado, variant: 'secondary' }
    );
  }
...
export default function OportunidadesPage() {
  ...
  const [editingLicitacionId, setEditingLicitacionId] = useState<number | null>(
    null,
  );
  const [editingOportunidadId, setEditingOportunidadId] = useState<
    number | null
  >(null);
  ...
  const loadData = async () => {
    try {
      ...
      const licitaciones: UnifiedItem[] =
        licResult.status === 'fulfilled'
      ...
      const oportunidades: UnifiedItem[] =
        opResult.status === 'fulfilled'
          ? (opResult.value.data.oportunidades || []).map(
              (o: Record<string, unknown>) => ({
                ...
                fecha:
                  (o.fecha_estimada_cierre as string) ||
                ...
                >
                  <CardContent className="p-4 space-y-2">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <Badge
                        variant={
                          item.tipo === 'licitacion' ? 'default' : 'outline'
                        }
                        className="shrink-0 text-xs"
                      >
                        {item.tipo === 'licitacion'
                          ? 'Licitación'
                          : 'Oportunidad'}
                      </Badge>
                    </div>
