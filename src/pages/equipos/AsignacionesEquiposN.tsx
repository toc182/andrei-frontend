...
export default function AsignacionesEquiposN() {
  const [asignaciones, setAsignaciones] = useState<AsignacionesState>({ alquiler: [], propios: [] })
  ...
  const loadRegistrosUso = async (asignacionId: number) => {
    try {
      const response = await api.get<ApiResponse<RegistroUsoExtended[]>>(`/registro-uso/asignacion/${asignacionId}`)
      if (response.data.success && response.data.data) {
  ...
  const handleAsignacionSubmit = async (data: AsignacionFormData) => {
    ...
    try {
      ...
      if (selectedAsignacion) {
        response = await api.put<ApiResponse<AsignacionExtended>>(`/asignaciones/${selectedAsignacion.id}`, data)
      } else {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsSubmitting(true)
    try {
      const response = await api.delete<ApiResponse<void>>(`/asignaciones/${selectedAsignacion.id}`)
      if (response.data.success) {
        await loadAsignaciones()
        setDeleteOpen(false)
        setFormOpen(false)
  ...
  // Renderizar tabla
  const renderTable = (asignacionesData: AsignacionExtended[], title: string, showRegistroBtn = false) => (
    <Card>
      <CardHeader>
