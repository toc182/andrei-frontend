...
const getEstadoBadge = (estado: string | undefined) => {
  ...
  const config = estado ? variants[estado] || { variant: 'secondary' as const, label: estado } : { variant: 'secondary' as const, label: 'N/A' }
...
export default function ProjectKPIsCards({ project }: ProjectKPIsCardsProps) {
  ...
  const gastado = (project?.datos_adicionales as { total_gastado?: number })?.total_gastado || 0
  ...
  const porcentajeAvance = (project?.datos_adicionales as { porcentaje_avance?: number })?.porcentaje_avance || 0
