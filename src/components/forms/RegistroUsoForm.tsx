...
const RegistroUsoForm = ({ asignacion, onClose, onSuccess }: RegistroUsoFormProps) => {
    ...
    return (
        <Dialog open={true} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle>Registro de Uso - {asignacion?.equipo_descripcion || 'Equipo'}</DialogTitle>
                </DialogHeader>
                                        ...
                                        <tr key={registro.id}>
                                            {asignacion?.tipo_cobro === 'hora' ? (
                                                ...
                                                <>
                                                    <td>{index + 1}</td>
                                                    <td>{new Date(registro.fecha_inicio).toLocaleDateString('es-PA')}</td>
                                                    <td>{registro.fecha_fin ? new Date(registro.fecha_fin).toLocaleDateString('es-PA') : '-'}</td>
                                                    <td>{registro.observaciones || '-'}</td>
                                                </>
                                            )}
                                        </tr>
