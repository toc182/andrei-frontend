...
import type { AsignacionExtended, RegistroUsoExtended } from '@/types';
...
interface RegistroUsoFormProps {
  asignacion: AsignacionExtended;
  onClose: () => void;
  onSuccess?: () => void;
}

const RegistroUsoForm = ({
  asignacion,
  onClose,
  onSuccess,
}: RegistroUsoFormProps) => {
  const [formData, setFormData] = useState<RegistroUsoFormData>({
    fecha_inicio: '',
    fecha_fin: '',
    cantidad: '',
    observaciones: '',
  });
  const [registros, setRegistros] = useState<RegistroUsoExtended[]>([]);
  ...
  const loadRegistros = async () => {
    try {
      ...
      const response = await api.get(
        `/registro-uso/asignacion/${asignacion.id}`,
      );
  ...
  return (
    <Dialog open={true} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            Registro de Uso - {asignacion?.equipo_descripcion || 'Equipo'}
          </DialogTitle>
        </DialogHeader>
        <form
        ...
        >
          ...
          >
            ...
            <span>
              {asignacion?.equipo_codigo} - {asignacion?.equipo_descripcion}
            </span>
          </div>
          ...
          >
            ...
            <span style={{ textTransform: 'capitalize' }}>
              {asignacion?.tipo_cobro === 'mes'
                ? 'Por Mes'
                : asignacion?.tipo_cobro === 'dia'
                  ? 'Por Día'
                  : asignacion?.tipo_cobro === 'semana'
                    ? 'Por Semana'
                    : asignacion?.tipo_cobro === 'hora'
                      ? 'Por Hora'
                      : asignacion?.tipo_cobro === 'costo_fijo'
                        ? 'Costo Fijo'
