...
const AsignacionForm = ({
      ...
      if (equiposRes.data.success) {
        ...
        if (asignacionesRes.data.success) {
          const asignaciones = asignacionesRes.data
            .data as AsignacionExtended[];
          const equiposAsignadosIds = asignaciones
            .filter(
              (a: AsignacionExtended) => !asignacion || a.id !== asignacion.id,
            ) // Excluir asignación actual si estamos editando
            .map((a: AsignacionExtended) => a.equipo_id);

          const disponibles = todosEquipos.filter(
            (e: EquipoExtended) => !equiposAsignadosIds.includes(e.id),
          );
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(
        apiError.response?.data?.message || 'Error al conectar con el servidor',
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async () => {
    ...
    const equipoNombre =
      equipos.find((e) => e.id === parseInt(formData.equipo_id))?.descripcion ||
      'este equipo';

    if (
      window.confirm(
        `¿Está seguro de que desea eliminar la asignación de ${equipoNombre}?`,
      )
    ) {
      try {
        setLoading(true);
  ...
  return (
    <Dialog open={true} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {asignacion ? 'Editar Asignación' : 'Nueva Asignación de Equipo'}
          </DialogTitle>
        </DialogHeader>
        <form
          id="asignacion-form"
        ...
        >
          ...
          {asignacion && (
            ...
            >
              <label style={{ marginBottom: 0, minWidth: '100px' }}>
                Equipo:
              </label>
              <span style={{ fontWeight: 'bold' }}>
                {
                  equipos.find((e) => e.id === parseInt(formData.equipo_id))
                    ?.codigo
                }{' '}
                -{' '}
                {
          ...
          <div className="asig-toggle-container">
            ...
            <button
              ...
              onClick={() =>
                setFormData((prev) => ({
                  ...prev,
                  incluye_operador: !prev.incluye_operador,
                }))
              }
            >
          ...
          <div className="asig-toggle-container">
            ...
            <button
              ...
              onClick={() =>
                setFormData((prev) => ({
                  ...prev,
                  incluye_combustible: !prev.incluye_combustible,
                }))
              }
            >
              <span className="asig-toggle-slider"></span>
