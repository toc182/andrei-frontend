...
import type { EquipoExtended, AsignacionExtended, Cliente, Project } from '@/types';
...
interface AsignacionFormProps {
    asignacion?: AsignacionExtended | null;
    onClose: () => void;
    onSuccess?: () => void;
}

const AsignacionForm = ({ asignacion = null, onClose, onSuccess }: AsignacionFormProps) => {
    ...
    useEffect(() => {
        ...
        if (asignacion) {
            setFormData({
                ...
                tarifa: asignacion.tarifa?.toString() || '',
                incluye_operador: asignacion.incluye_operador || false,
                costo_operador: asignacion.costo_operador?.toString() || '',
                incluye_combustible: asignacion.incluye_combustible || false,
                costo_combustible: asignacion.costo_combustible?.toString() || '',
                ajuste_monto: asignacion.ajuste_monto?.toString() || '',
    ...
    const loadInitialData = async () => {
        try {
            ...
            if (equiposRes.data.success) {
                const todosEquipos = equiposRes.data.data as EquipoExtended[];
                setEquipos(todosEquipos);

                // Filtrar equipos que ya están asignados (excepto si estamos editando)
                if (asignacionesRes.data.success) {
                    const asignaciones = asignacionesRes.data.data as AsignacionExtended[];
                    const equiposAsignadosIds = asignaciones
                        .filter((a: AsignacionExtended) => !asignacion || a.id !== asignacion.id) // Excluir asignación actual si estamos editando
                        .map((a: AsignacionExtended) => a.equipo_id);

                    const disponibles = todosEquipos.filter((e: EquipoExtended) => !equiposAsignadosIds.includes(e.id));
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            setError(apiError.response?.data?.message || 'Error al conectar con el servidor');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async () => {
        ...
        const equipoNombre = equipos.find(e => e.id === parseInt(formData.equipo_id))?.descripcion || 'este equipo';
    ...
    return (
        <Dialog open={true} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle>{asignacion ? "Editar Asignación" : "Nueva Asignación de Equipo"}</DialogTitle>
                </DialogHeader>
                ...
                {asignacion && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
                        ...
                        <span style={{ fontWeight: 'bold' }}>
                            {equipos.find(e => e.id === parseInt(formData.equipo_id))?.codigo} - {equipos.find(e => e.id === parseInt(formData.equipo_id))?.descripcion}
                        </span>
                    </div>
                )}
