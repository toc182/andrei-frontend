  ...
  .refine(
    (data) => {
      ...
      if (data.datos_adicionales.es_consorcio) {
        const total = data.datos_adicionales.socios.reduce(
          (sum, socio) => sum + socio.porcentaje,
          0,
        );
...
const ProjectFormNew = ({
  ...
  // React Hook Form
  const form: UseFormReturn<ProjectFormData> = useForm<ProjectFormData>({
      ...
      if (response.data.success) {
        ...
        form.reset({
          ...
          fecha_inicio: project.fecha_inicio
            ? project.fecha_inicio.split('T')[0]
            : '',
          ...
          monto_contrato_original: project.monto_contrato_original
            ? String(project.monto_contrato_original)
            : '',
          presupuesto_base: project.presupuesto_base
            ? String(project.presupuesto_base)
            : '',
  ...
  const handlePartnerChange = (
    ...
    const updatedSocios = currentSocios.map((socio, i) =>
      i === index
        ? {
            ...socio,
            [field]: field === 'porcentaje' ? parseFloat(value) || 0 : value,
          }
        : socio,
    );
  ...
  const handleSubmit = async (data: ProjectFormData) => {
    try {
      ...
      const submitData: Record<string, unknown> = {
        ...
        monto_contrato_original: data.monto_contrato_original
          ? parseFloat(data.monto_contrato_original)
          : null,
        presupuesto_base: data.presupuesto_base
          ? parseFloat(data.presupuesto_base)
          : null,
      ...
      const datosAdicionales = submitData.datos_adicionales as Record<
        string,
        unknown
      >;
      Object.keys(datosAdicionales).forEach((key) => {
        if (datosAdicionales[key] === '') {
          datosAdicionales[key] = null;
        }
      });
      ...
      setError(
        apiError.response?.data?.message ||
          'Error de conexión al guardar el proyecto',
      );
    } finally {
      setLoading(false);
    }
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <RadioGroup
                        ...
                        onValueChange={(value) =>
                          field.onChange(value === 'consorcio')
                        }
                        disabled={loading}
              ...
              {watchEsConsorcio && (
                <div className="space-y-4 p-4 border rounded-lg bg-muted/50">
                  ...
                  {watchSocios.map((socio, index) => (
                    <div
                      key={index}
                      className="space-y-3 md:space-y-0 md:grid md:grid-cols-[1fr_120px_40px] md:gap-3 md:items-start"
                    >
                      {/* Nombre del Socio */}
                      <div>
                        ...
                        <Input
                          ...
                          value={socio.nombre}
                          onChange={(e) =>
                            handlePartnerChange(index, 'nombre', e.target.value)
                          }
                  ...
                  <div className="flex items-center justify-center gap-2 p-3 bg-background rounded-md border">
                    ...
                    <span className="text-lg font-bold">
                      {watchSocios
                        .reduce(
                          (sum, socio) => sum + (socio.porcentaje || 0),
                          0,
                        )
                        .toFixed(2)}
                      %
