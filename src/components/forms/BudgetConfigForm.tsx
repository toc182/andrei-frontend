...
export default function BudgetConfigForm({ projectId, isOpen, onClose, onSave }: BudgetConfigFormProps) {
  ...
  const [pendingNewCategories, setPendingNewCategories] = useState<PendingCategory[]>([]) // New categories to create
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (budgetResponse.data.success && budgetResponse.data.categories) {
          budgetResponse.data.categories.forEach((cat: { project_category_id: number; presupuesto_actual?: number; presupuesto_inicial?: number }) => {
        ...
        const savedTotal = Object.values(savedBudgets).reduce((sum, val) => sum + (parseFloat(String(val)) || 0), 0)
        if (savedTotal > 0) {
  ...
  const visibleCategories: VisibleCategory[] = [
    ...
    ...pendingNewCategories.map(c => ({ ...c, id: c.tempId as unknown as number } as VisibleCategory))
  ]
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(apiError.response?.data?.message || 'Error al guardar el presupuesto')
    } finally {
      setLoading(false)
    }
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <Input
              ...
              placeholder="0.00"
              value={totalBudget}
              onChange={(e: ChangeEvent<HTMLInputElement>) => setTotalBudget(e.target.value)}
              required
            />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e: ChangeEvent<HTMLInputElement>) => setNewCategory(prev => ({ ...prev, nombre: e.target.value }))}
                    />
                  </div>
                  <div>
                ...
                {visibleCategories.map((category) => (
                  <div key={category.id} className="grid grid-cols-[auto_1fr_2fr_auto] gap-3 items-center">
                    ...
                    <Input
                      ...
                      onChange={(e: ChangeEvent<HTMLInputElement>) => handleBudgetChange(category.id, e.target.value)}
                    />
                    <button
                      type="button"
